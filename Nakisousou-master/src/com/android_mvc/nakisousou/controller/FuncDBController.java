...
import com.android_mvc.nakisousou.activities.func_db.SampleTabHostActivity;
import com.android_mvc.nakisousou.activities.func_map.SampleMapActivity;
import com.android_mvc.nakisousou.activities.func_net.HttpNetActivity;
import com.android_mvc.nakisousou.activities.main.TopActivity;
import com.android_mvc.nakisousou.db.dao.LocationLogDAO;
import com.android_mvc.nakisousou.domain.DBDeleteAction;
import com.android_mvc.nakisousou.domain.DBEditAction;
import com.android_mvc.nakisousou.domain.DBUpdateAction;
import com.android_mvc.nakisousou.domain.DBUpdateUsoppoiAction;
...
public class FuncDBController extends BaseController
{
    ...
    public static void submit(final DBListActivity activity, String action_type, final Long friend_id)
    {
        ...
        if( "UPDATE_NAITESOU".equals(action_type) )
        {
            // DB更新
        ...
        if( "UPDATE_USOPPOI".equals(action_type) )
        {
            // DB更新
            new ControlFlowDetail<DBListActivity>( activity )
                .setBL( new BLExecutor(){
                    ...
                    public ActionResult doAction()
                    {
                        return new DBUpdateUsoppoiAction( activity, friend_id ).exec();
                    }
                })
                .onBLExecuted(
                    new RoutingTable().map("success", DBListActivity.class )
     ...
     * 最新の位置情報を取得する
     */
    public static void submit(final SampleMapActivity activity)
    {
        // DB参照
        new ControlFlowDetail<SampleMapActivity>( activity )
            .setBL( new BLExecutor(){
                @Override
                public ActionResult doAction()
                {
                    return new ActionResult()
                        .add("recent_locations", new LocationLogDAO(activity).findAll())
                        .setRouteId("success")
                    ;
                }
            })
            .onBLExecuted( STAY_THIS_PAGE_ALWAYS )
            .startControl();
        ;
